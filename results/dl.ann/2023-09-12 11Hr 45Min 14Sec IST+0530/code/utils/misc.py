...
def build_trainable_reward_model(config, checkpoint, build_model):

    reward_model = build_model(config)

    if config['model']['__name__'].endswith('ann'):
        print("Loading the encoder of reward model from policy checkpoint...")
        # Copy the encoder state_dict from the checkpoint to the new model
        ...
        print("Making the encoder parameters of reward model untrainable...")
        # Set requires_grad to False for reward_head parameters
        for param in reward_model.encoder.parameters():
            param.requires_grad = False

    elif config['model']['__name__'].endswith('vit'):
        if not list(reward_model.state_dict().keys())[0].startswith('vit_encoder'):
            print("Loading the patch embedding, transformer and latent module of reward model from policy checkpoint...")
            # Copy the encoder state_dict from the checkpoint to the new model
            patch_embedding_state_dict = {k.replace('to_patch_embedding.', ''): v 
                                        for k, v in checkpoint.items() if k.startswith('to_patch_embedding')}
            transformer_state_dict = {k.replace('transformer.', ''): v 
                                    for k, v in checkpoint.items() if k.startswith('transformer')}
            to_latent_state_dict = {k.replace('to_latent.', ''): v 
                                    for k, v in checkpoint.items() if k.startswith('to_latent')}
            reward_model.to_patch_embedding.load_state_dict(patch_embedding_state_dict)
            reward_model.transformer.load_state_dict(transformer_state_dict)
            reward_model.to_latent.load_state_dict(to_latent_state_dict)

            print("Making the patch embedding, transformer and latent module parameters of reward model untrainable...")
            # Set requires_grad to False for reward_head parameters
            for param in reward_model.to_patch_embedding.parameters():
                param.requires_grad = False
            for param in reward_model.transformer.parameters():
                param.requires_grad = False
            for param in reward_model.to_latent.parameters():
                param.requires_grad = False
        else:
            print("Loading the vit_encoder of reward model from policy checkpoint...")
            # Copy the encoder state_dict from the checkpoint to the new model
            vit_encoder_state_dict = {k.replace('vit_encoder.', ''): v 
                                      for k, v in checkpoint.items() if k.startswith('vit_encoder')}
            reward_model.vit_encoder.load_state_dict(vit_encoder_state_dict)

            print("Making the vit_encoder parameters of reward model untrainable...")
            # Set requires_grad to False for reward_head parameters
            for param in reward_model.vit_encoder.parameters():
                param.requires_grad = False

    print("Reward model built successfully!")
    return reward_model

def logits_from_probability(probs):
    # Calculate the log-softmax
    log_probs = F.log_softmax(probs, dim=-1)

    # Use the formula to get the logits back
    logits = log_probs + torch.logsumexp(log_probs, dim=-1)
    return logits
